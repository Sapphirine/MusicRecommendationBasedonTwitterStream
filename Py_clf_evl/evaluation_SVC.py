...
#Train the classifier and evaluation
#SVC_model= LinearSVC(C=0.5).fit(corpus_data_features_nd, train_data_df.Sentiment)
SVC_model = LinearSVC(C=0.5).fit(X_train, y_train)

#test and classfier and 
y_pred = SVC_model.predict(X_test)
scores = SVC_model.score(X_test, y_test)
c_scores = cross_val_score(SVC_model, 
                           corpus_data_features_nd, 
                           train_data_df.Sentiment )
